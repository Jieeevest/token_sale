...
contract("BaleToken", function (accounts) {
  var tokenInstance;
  ...
  it("initializes the contract with the correct values", function () {
    return BaleToken.deployed()
      ...
      .then(function (name) {
        assert.equal(name, "Bale Token", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "BLT", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "Bale Token v1.0", "has the correct standard");
      });
  });
  ...
  it("approve tokens for delegate transfer", function () {
    return BaleToken.deployed()
      .then(function (instance) {
        tokenInstance = instance;
      ...
      .then(function (success) {
        assert.equal(success, true, "approve it returns true");
    ...
    return BaleToken.deployed()
      ...
      .then(function (allowance) {
        assert.equal(
          allowance.toNumber(),
          0,
          "deducts the emount from the allowance"
        );
      });
  });
});
